...
public interface ApplicationRepository extends JpaRepository<Application, Long> {
    ...
    @Query("SELECT new com.pacto.api.application.dto.ApplicationResponse(" +
            "a.applicationId, a.campaignId, a.bloggerId, u.email, a.status, a.createdAt, a.updatedAt) " +
            "FROM Application a JOIN User u ON a.bloggerId = u.userId " +
            "WHERE a.campaignId IN :campaignIds ORDER BY a.createdAt DESC")
    List<ApplicationResponse> findRecentByCampaignIdIn(@Param("campaignIds") List<Long> campaignIds, Pageable pageable);

    long countByCampaignIdInAndStatus(List<Long> campaignIds, ApplicationStatus status);
}
